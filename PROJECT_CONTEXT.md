...
**BELUM DIKERJAKAN / dipertimbangkan tapi sengaja dilewati karena keterbatasan waktu (tidak ditanyakan ke user, next thing kalau ada laporan masalah):**
- Tidak ada indikator visual "read-only" eksplisit di halaman-halaman yang tombol edit-nya disembunyikan (misal badge "View Only" di header) — cukup mengandalkan tombol Add/Edit/Delete yang otomatis hilang lewat `@can`.
- Tidak sempat audit SEMUA view Blade satu-satu untuk tombol yang mungkin masih muncul padahal user tidak boleh klik (terutama di master-data/organization views dan skills/positions views) — secara backend semua sudah aman (403 kalau dipaksa), tapi UX bisa saja masih menampilkan tombol yang ujung-ujungnya gagal. Kalau user lapor ada tombol "hantu" seperti itu, itu next thing to fix.
- Fitur "Add User" di User Management BELUM bisa langsung link ke Employee/posisi dalam satu form — admin harus buka Employee form terpisah dan set "Linked User". Kalau user mau alur satu-form-langsung-jadi, ini bisa ditambah.
