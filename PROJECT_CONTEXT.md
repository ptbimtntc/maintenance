...
Brief asli membagi pekerjaan jadi **7 fase**. Semua 7 fase **sudah selesai dan di-commit**. Setelah itu user minta lanjutkan **4 item Roadmap** di README (Settings, Notifikasi, Export .xlsx, Kalender visual) — **semua 4 sudah selesai DAN sudah di-commit**. Setelah itu, user minta fitur besar **di luar Roadmap**: sistem otorisasi berjenjang (hierarki atasan-bawahan), izin edit per-user-per-menu, User Management admin, akses Guest read-only, plus follow-up read-only badge & link-employee-saat-create-user — **semua sudah selesai dan di-commit** (lihat bagian "Fitur besar: Hierarchical Permissions & Guest Access"). Setelah itu, user minta **restrukturisasi field profil Employee** (hapus beberapa field lama, tambah field klasifikasi baru + foto) — lihat bagian "Perubahan field profil Employee" di bawah — **sudah selesai, 157 test lulus, sudah diverifikasi browser, TAPI BELUM DI-COMMIT** di titik penulisan ini (cek `git log`/`git status` untuk kepastian, catatan ini bisa jadi basi).
...
- `bin/start.sh` — script convenience start MySQL + server. Sekarang juga menjalankan `php artisan storage:link` (idempotent, dicek dulu apakah symlink sudah ada) supaya foto karyawan bisa diakses via URL publik setelah restart Codespace.

## Perubahan field profil Employee (BELUM DI-COMMIT)

User minta ubah field di tab Overview halaman Employee (dan form create/edit-nya):

**Dihapus dari tampilan Overview + form create/edit** (kolom database TETAP ADA, cuma tidak lagi ditampilkan/diminta — non-destruktif, supaya tidak merusak data existing atau fitur lain yang masih pakai kolom itu, misal filter di Employees index & Skill Matrix masih pakai `maintenance_area_id`): Preferred Name, Education, Manager (`manager_id` — tidak perlu diisi manual karena "otomatis segaris dengan supervisornya", TAPI saya TIDAK bikin logika auto-compute-nya, cuma menghapus field-nya dari UI karena user bilang "tidak perlu mengisi", bukan "hitung otomatis dari rantai supervisor" — kalau nanti user mau field Manager BENERAN dihitung otomatis dari supervisor-nya-supervisor, itu perlu ditambah terpisah), Division, Maintenance Area, Work Location, Years of Experience.

**Ditambahkan** (perlu 2x AskUserQuestion untuk klarifikasi karena awalnya ambigu — lihat detail di bawah):
- **Photo** — `photo_path` kolom SUDAH ADA dari awal tapi TIDAK PERNAH dipakai; sekarang beneran bisa upload. Disimpan di disk `public` (BUKAN `local`/private seperti sertifikat, karena foto profil bukan data sensitif) di folder `employee-photos`, diakses via `Storage::disk('public')->url(...)`. **PENTING**: perlu `php artisan storage:link` supaya `public/storage` symlink ada — sudah dijalankan di Codespace ini, dan sudah ditambahkan ke `bin/start.sh` (idempotent) supaya tidak hilang kalau environment baru/di-rebuild.
- **Business Unit** — master data baru (`business_units` tabel), field `business_unit_id` di employees. Nilai seed: Tire Cord, Half Product, Dramix (persis sesuai request user).
- **Skill Position** — master data baru (`skill_positions` tabel), field `skill_position_id`. User klarifikasi ini BEDA dari Position biasa — Position = jabatan struktural, Skill Position = spesialisasi teknis. Nilai seed: Mechanical, Electrical, Instrument.
- **Management (BC/WCM)** — BUKAN master data (cuma 2 nilai tetap, tidak perlu CRUD terpisah), disimpan sebagai kolom string `workforce_category` di employees, validasi `Rule::in(['BC','WCM'])`. Label ditentukan dari `Employee::WORKFORCE_CATEGORIES` const (`'BC' => 'Blue Collar (BC)'`, `'WCM' => 'White Collar Management (WCM)'`). User klarifikasi: BC = Blue Collar (setara operator/technician), WCM = White Collar Management (staff ke atas) — ini KLASIFIKASI STATUS KARYAWAN, bukan program pelatihan.
- **Employment Source** — master data baru (`employment_sources` tabel), field `employment_source_id`. Nilai seed: Bekaert, Brexa, Gokko, Mahkota, TSS (nama vendor/agency perekrutan).

**File yang berubah**: 4 migrasi baru (`create_business_units_table`, `create_skill_positions_table`, `create_employment_sources_table`, `add_new_fields_to_employees_table` — urutan timestamp PENTING karena employees migration nambah FK ke 3 tabel lain, harus jalan SETELAHNYA), 3 model+factory baru (`BusinessUnit`, `SkillPosition`, `EmploymentSource` — pola identik dengan `Department`), `config/master_data.php` (3 entry baru: `business-units`, `skill-positions`, `employment-sources` — otomatis dapat CRUD generic + otomatis muncul di `/organization` landing TANPA perlu ubah view, karena sistemnya sudah config-driven), `app/Models/Employee.php` (const `WORKFORCE_CATEGORIES`, relasi `businessUnit()`/`skillPosition()`/`employmentSource()`, Fillable ditambah field baru), `StoreEmployeeRequest`/`UpdateEmployeeRequest` (validasi field baru ditambah, validasi field yang dihapus dari form DIHAPUS juga dari rules), `EmployeeController` (formOptions() diupdate, `store()`/`update()` sekarang pisah `$request->safe()->except('photo')` lalu handle upload foto terpisah lewat method baru `storeUploadedPhoto()` — pola sama seperti `CertificateController::storeUploadedFile()`), `resources/views/employees/_form.blade.php` (field lama dihapus, field baru ditambah, ada `enctype="multipart/form-data"` yang HARUS ditambah di `create.blade.php` dan `edit.blade.php` juga — kalau lupa, upload foto akan gagal diam-diam), `resources/views/employees/show.blade.php` (foto ditampilkan di header profil dengan fallback inisial nama kalau belum ada foto, grid Overview diupdate), `database/seeders/MasterDataSeeder.php` + `EmployeeSeeder.php` + `EmployeeFactory.php` (seed 3 master data baru dengan nilai persis sesuai request user, employee demo/seed dapat nilai realistis bukan random dari factory kosong — PENTING: kalau nambah employee lewat factory tanpa override field baru, factory create `BusinessUnit::factory()`/dst BARU secara acak, makanya `EmployeeSeeder`'s `$additional` state HARUS eksplisit pakai `$businessUnits->random()->id` dkk, bukan dibiarkan ke default factory, supaya tidak nambah sampah master data acak).

**Test baru**: 3 test ditambahkan ke `tests/Feature/EmployeeManagementTest.php` — create employee dengan field klasifikasi baru, validasi `workforce_category` menolak nilai tidak valid, upload foto beneran tersimpan ke disk (pakai `Storage::fake('public')` + `UploadedFile::fake()->create(..., 'image/jpeg')` BUKAN `->image()` karena **GD extension tidak terpasang di PHP Codespace ini** — `->image()` butuh GD untuk generate gambar asli, `->create()` cukup untuk lolos validasi `'image'` rule tanpa perlu GD). Total suite **157 test lulus**.

**Sudah diverifikasi lewat browser (Playwright, MySQL asli)**: buka employee existing, overview tampilkan field baru dengan benar (Business Unit, Skill Position, Management, Employment Source) dan field lama beneran hilang; form edit menampilkan semua field baru dan field lama beneran tidak ada lagi; submit form dengan foto baru → foto tersimpan dan tampil di header profil (pakai file JPEG dummy minimal, jadi gambarnya terlihat rusak di screenshot tapi itu cuma karena data uji, bukan bug); halaman `/organization` dan `/organization/business-units` menampilkan 3 master data baru dengan benar. Data uji sudah dibersihkan, dev DB di-`migrate:fresh --seed` ulang.

**BELUM DIKERJAKAN / dipertimbangkan tapi sengaja dilewati**:
- Field "Manager" TIDAK dihitung otomatis dari rantai supervisor (misal `$employee->supervisor?->supervisor`) — cuma dihapus dari form/tampilan sesuai instruksi literal user ("tidak perlu mengisi"). Kolom `manager_id` dan relasi `manager()` masih ada di model tapi sekarang benar-benar tidak terpakai di mana pun. Kalau user ternyata mau field itu tetap tampil tapi dihitung otomatis, perlu ditambahkan sebagai computed accessor, bukan field database.
- Tidak menghapus kolom `preferred_name`, `education`, `division_id`, `maintenance_area_id`, `location_id`, `years_of_experience`, `manager_id` dari database — sengaja dipertahankan (non-destruktif) karena beberapa (`division_id`, `maintenance_area_id`, `location_id`) masih dipakai di tempat lain (filter Employees index, Skill Matrix, dashboard) yang TIDAK diminta user untuk dihapus, cuma dihapus dari form/profil individual karyawan. Kalau user mau konsep Division/Maintenance Area/Work Location dihapus TOTAL dari sistem (bukan cuma dari profil karyawan), itu perubahan jauh lebih besar yang perlu didiskusikan terpisah.
- Belum ada validasi/UI untuk menghapus foto yang sudah diupload (cuma bisa diganti dengan upload baru, belum ada tombol "Remove Photo").

## Status akhir

- Semua 4 item Roadmap (Settings, Notifikasi, Export .xlsx, Kalender visual) — **sudah di-commit**, tidak ada sisa.
- Fitur hierarki/menu-edit/guest/user-management + follow-up (read-only badge, link-employee-saat-create-user) — **sudah di-commit** (2 commit: `4914fce` dan `d3d58b9`).
- Perubahan field profil Employee (hapus field lama, tambah Business Unit/Skill Position/Management/Employment Source/Photo) — selesai + teruji (157 test lulus + verifikasi browser), **belum di-commit** di titik penulisan ini.
- **Langkah berikutnya**: commit perubahan field Employee ini (satu commit, semua perubahan di working tree adalah satu kesatuan fitur ini), lalu tanyakan ke user apakah 3 poin "belum dikerjakan" di atas (computed Manager field, hapus kolom lama dari DB, tombol Remove Photo) perlu ditindaklanjuti.
