            ...
            <x-dropdown align="right" width="80">
                <x-slot name="trigger">
                    <button type="button" class="relative text-neutral-400 hover:text-neutral-600" title="{{ $unreadCount > 0 ? $unreadCount.' unread notification(s)' : 'Notifications (no unread items)' }}">
                        ...
                        @if ($unreadCount > 0)
                            <span class="absolute -right-1 -top-1 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-danger-600 px-1 text-[10px] font-semibold text-white">
                    ...
                    <div class="flex items-center justify-between border-b border-neutral-100 px-4 py-2">
                        ...
                        @if ($unreadCount > 0)
                            <form method="POST" action="{{ route('notifications.read-all') }}">
                                @csrf
                                <button type="submit" class="text-xs font-medium text-accent-600 hover:text-accent-800">
                                    Mark all as read
                                </button>
                            </form>
                        @endif
