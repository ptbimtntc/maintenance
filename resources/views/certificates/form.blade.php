    ...
    <div class="max-w-2xl rounded-lg border border-neutral-200 bg-white p-6 shadow-md">
        <form method="POST" action="{{ $isEdit ? route('employees.certificates.update', [$employee, $certificate]) : route('employees.certificates.store', $employee) }}" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select id="certificate_type_id" name="certificate_type_id" class="mt-1 block w-full rounded-md border-neutral-300 text-sm">
                            <option value="">—</option>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="related_training_program_id" name="related_training_program_id" class="mt-1 block w-full rounded-md border-neutral-300 text-sm">
                            <option value="">—</option>
                ...
                <div>
                    ...
                    <select id="verification_status" name="verification_status" class="mt-1 block w-full rounded-md border-neutral-300 text-sm">
                    ...
                    <p class="mt-1 text-xs text-neutral-400">A certificate is never assumed valid just because a file was uploaded — mark it verified once you've checked it.</p>
                </div>

                <div>
                    <x-input-label for="file" value="Certificate File (optional, PDF/JPG/PNG, max 5MB)" />
                    <input id="file" type="file" name="file" accept=".pdf,.jpg,.jpeg,.png" class="mt-1 block w-full rounded-md border-neutral-300 text-sm" />
                    <x-input-error :messages="$errors->get('file')" class="mt-1" />
                    @if ($isEdit && $certificate->file_path)
                        <p class="mt-1 text-xs text-neutral-500">Current file: {{ $certificate->file_original_name }} — uploading a new one will replace it.</p>
                    @endif
                </div>
            ...
            <div class="mt-6 flex justify-end gap-2">
                ...
                <button type="submit" class="rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">Save Certificate</button>
            </div>
        </form>
    </div>
</x-app-layout>
