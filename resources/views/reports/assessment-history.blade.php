<x-app-layout>
    ...
    <div class="space-y-4">
        ...
        <x-filter-panel :active-count="$filterActiveCount">
            <form method="GET" class="flex gap-3 rounded-lg border border-neutral-200 bg-white p-4">
                <input type="text" name="employee_search" value="{{ $filters['employee_search'] ?? '' }}" placeholder="Search employee..." class="block w-full rounded-md text-sm {{ filled($filters['employee_search'] ?? null) ? $activeFieldClass : $defaultFieldClass }}" />
                <button type="submit" class="shrink-0 rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">Filter</button>
                <a href="{{ route('reports.assessment-history') }}" class="shrink-0 rounded-md border border-neutral-300 px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50">Reset</a>
            </form>
        </x-filter-panel>
