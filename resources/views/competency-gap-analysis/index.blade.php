<x-app-layout>
    ...
    <div class="space-y-8">
        ...
        <x-filter-panel :active-count="$filterActiveCount">
            <form method="GET" action="{{ route('competency-gap-analysis.index') }}" class="grid grid-cols-1 gap-2.5 rounded-lg border border-neutral-200 bg-white p-3 shadow-sm sm:grid-cols-4">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('competency-gap-analysis.index') }}" class="rounded-md border border-neutral-300 px-3.5 py-1.5 text-sm font-medium text-neutral-600 transition shadow-sm hover:border-accent-400 hover:bg-accent-50 hover:text-accent-700">Reset</a>
                </div>
            </form>
        </x-filter-panel>
