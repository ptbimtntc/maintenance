    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            @can(\App\Enums\PermissionName::ManageJobDescriptions->value)
                <a href="{{ route('job-descriptions.create') }}" class="rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-brand-700">
                    Add Job Description
                </a>
            @endcan
        </div>
        ...
        <x-filter-panel :active-count="$filterActiveCount">
            <form method="GET" action="{{ route('job-descriptions.index') }}" class="grid grid-cols-1 gap-2.5 rounded-lg border border-neutral-200 bg-white p-3 shadow-sm sm:grid-cols-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('job-descriptions.index') }}" class="rounded-md border border-neutral-300 px-3.5 py-1.5 text-sm font-medium text-neutral-600 transition shadow-sm hover:border-accent-400 hover:bg-accent-50 hover:text-accent-700">Reset</a>
                </div>
            </form>
        </x-filter-panel>
